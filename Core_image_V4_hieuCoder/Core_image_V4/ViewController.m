...
- (IBAction)ResizeImage:(NSMutableArray *)images
{
    ...
    dispatch_queue_t myQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND,0 ) ;        //dispatch_queue_create("hieuCoder", DISPATCH_QUEUE_SERIAL);// Day la dispatch_private_queue
    ...
    for (int i = 0; i < [_images count]; i++)
    {
        dispatch_async(myQueue,
            ...
            dispatch_async(dispatch_get_main_queue(),
                         ^{
                            CGRect frame          = CGRectMake(10+50*i,60*i,img.size.width, img.size.height);
                            _imageResized         = [[UIImageView alloc]initWithFrame:frame];
                            [_imageResized setImage:img];
                            [self.view addSubview:_imageResized];
//NSLog(@" Size goc %1.0f %1.0f ",temp.size.width,temp.size.height);
